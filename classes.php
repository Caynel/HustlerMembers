...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['class_name'], $_POST['student_name'], $_POST['student_email'], $_POST['class_schedule'])) {
    // Sanitize inputs
    ...
    $classScheduleTimestamp = intval($_POST['class_schedule']);
    $classSchedule = date("F j, Y - h:00 A", $classScheduleTimestamp);
    ...
    if (!isset($_SESSION['enrolledClasses'])) {
        $_SESSION['enrolledClasses'] = [];
    }

    // Add enrollment to session
    $_SESSION['enrolledClasses'][] = $enrollment;

    // Redirect to enrolled.php to show enrolled classes
...
<body>
    ...
    <?php if ($showForm): ?>
        <div class="enroll-form-bg" onclick="if(event.target === this) window.location.href='classes.php';">
            <form class="enroll-form" method="post" action="classes.php" onclick="event.stopPropagation();">
